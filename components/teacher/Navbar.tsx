...
export default function TeacherNavbar({
  ...
  const navigationItems: NavItem[] = [
    { name: "Dashboard", icon: LayoutDashboard, path: "/pages-Teacher/TeacherDashboard" },
    { name: "My Quizzes", icon: BookOpen, path: "/pages-Teacher/MyQuiz" },
    { name: "Create Quiz", icon: FileQuestion, path: "/pages-Teacher/CreateQuiz" },
    { name: "Students", icon: Users, path: "/pages-Teacher/ShowStudents" },
  ...
  return (
    <>
      ...
      {/* ✅ Mobile toggle button on right side (only visible on small screens) */}
      <button
        onClick={() => onMobileToggle(!mobileOpen)}
        className="fixed top-5 right-5 z-[60] md:hidden bg-white/90 text-indigo-900 rounded-full p-2 shadow-lg border border-gray-200"
      >
        {mobileOpen ? (
          <ChevronLeft className="w-5 h-5" />
        ) : (
