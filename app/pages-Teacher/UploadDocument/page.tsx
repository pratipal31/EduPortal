...
import { Upload, FileText, Wand2, Loader, CheckCircle, AlertCircle, Trash2, Edit3, Save } from "lucide-react"
...
export default function UploadDocumentPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selectedFilesFromInput = Array.from(e.target.files || [])
    if (selectedFilesFromInput.length > 0) {
      setSelectedFiles(selectedFilesFromInput)
      // Auto-generate titles for new files if not already provided
      const newTitles = selectedFilesFromInput.map((file, index) => titles[index] || file.name.replace(/\.[^/.]+$/, ""))
      setTitles(newTitles)
    }
  }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      selectedFiles.forEach((file, index) => {
        ...
        formData.append(`title_${index}`, titles[index] || file.name.replace(/\.[^/.]+$/, ""))
      })
      ...
      // Update documents status for all successfully uploaded files
      setDocuments((prev) =>
        prev.map((doc) => {
          const matchingUpload = data.results.find((r: any) => r.title === doc.title)
          if (matchingUpload) {
            return { ...doc, status: "success", chunks: matchingUpload.chunks }
          }
          return doc
        }),
      )

      const successTitles = data.results.map((r: any) => r.title).join(", ")
      setSuccess(`${data.results.length} document(s) uploaded successfully! Total chunks: ${data.totalChunks}`)
      setSelectedFiles([])
      setTitles([])
      ...
      // Mark all docs as error
      setDocuments((prev) =>
        prev.map((doc) => (newDocs.find((d) => d.id === doc.id) ? { ...doc, status: "error", error: errorMsg } : doc)),
      )
      setError(errorMsg)
    }
  }
  ...
  const uploadedChunks = documents
    .filter((doc) => doc.status === "success")
    .reduce((total, doc) => total + doc.chunks, 0)
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start space-x-2 sm:space-x-3">
                      ...
                      {editingIndex === index ? (
                        ...
                        <p className="flex-1 font-semibold text-gray-900 text-sm sm:text-base">
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-gray-600 text-sm sm:text-base">
            Upload multiple teaching materials and let AI create custom quizzes
          </p>
        </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 lg:p-8">
              <div className="flex items-center space-x-2 sm:space-x-3 mb-4 sm:mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-600">Add multiple learning materials</p>
                </div>
              </div>
              ...
              <div className="space-y-3 sm:space-y-4">
                    ...
                    {selectedFiles.length > 0 ? (
                      <div>
                        ...
                        <p className="text-gray-500 text-xs mt-1">
                          {(selectedFiles.reduce((total, f) => total + f.size, 0) / 1024).toFixed(2)} KB total
                        </p>
                      </div>
                    ) : (
                      <div>
                        <p className="text-gray-600 font-medium text-xs sm:text-sm">Click to upload or drag and drop</p>
                        <p className="text-gray-400 text-xs mt-1">PDF, DOCX, or TXT (Max 50MB per file)</p>
                      </div>
                    )}
                  </label>
                </div>
                ...
                >
                  ...
                  <span>Upload {selectedFiles.length > 0 ? selectedFiles.length : ""} Document(s)</span>
                </button>
              </div>
              ...
              {success && (
                <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-green-50 border border-green-200 rounded-lg flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-green-700 text-xs mt-1">Ready for quiz generation! ➡️</p>
                  </div>
                </div>
              )}
              ...
              {documents.length > 0 && (
                <div className="mt-6 space-y-2 border-t pt-4">
                  <h3 className="text-sm font-semibold text-gray-700 mb-3">Uploaded Documents ({documents.length})</h3>
                      ...
                      >
                        <div className="flex items-center space-x-2 flex-1 min-w-0">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-xs sm:text-sm font-medium text-gray-900 truncate">{doc.title}</p>
                            {doc.status === "success" && (
                              <p className="text-xs text-green-600 font-medium">✓ {doc.chunks} chunks</p>
                            )}
                        ...
                        <button
                          ...
                          className="ml-2 p-1.5 text-red-600 hover:bg-red-50 rounded transition-colors disabled:opacity-50 flex-shrink-0"
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 lg:p-8">
              ...
              {uploadedChunks > 0 ? (
                <div className="space-y-4">
                  <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-lg border border-purple-200">
                    <p className="text-xs sm:text-sm font-semibold text-gray-700">
                      📊 Total Chunks Ready: <span className="text-purple-600">{uploadedChunks}</span>
                    </p>
                    <p className="text-xs text-gray-600 mt-1">
                      From {documents.filter((d) => d.status === "success").length} document(s)
                    </p>
                  </div>
                  <button
                ...
                <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-6 sm:p-8 text-center">
                  ...
                  <p className="text-gray-500 text-xs sm:text-sm">Upload documents on the left to generate quizzes</p>
                </div>
              )}

              {/* Info Box */}
              <div className="mt-4 sm:mt-6 bg-purple-50 border border-purple-200 rounded-lg p-3 sm:p-4">
                <h3 className="font-semibold text-purple-900 text-xs sm:text-sm mb-2">✨ AI Quiz Features:</h3>
                <ul className="space-y-1 text-xs text-purple-800">
                  <li>• Questions from ALL uploaded documents</li>
                  <li>• Multiple question types available</li>
                  <li>• Adjustable difficulty levels</li>
                  <li>• Review & edit before publishing</li>
                  <li>• Delete any document anytime</li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-4 sm:p-6">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Quiz Title *</label>
                  <input
                    type="text"
              ...
              {/* Difficulty Level */}
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-3">Difficulty Level</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Number of Questions</label>
                  <select
                    value={quizConfig.numQuestions}
                    onChange={(e) => setQuizConfig({ ...quizConfig, numQuestions: Number.parseInt(e.target.value) })}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setQuizConfig({ ...quizConfig, duration: Number.parseInt(e.target.value) })}
                    className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 text-sm sm:text-base"
                  >
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Passing Score</label>
                  <select
                    value={quizConfig.passingScore}
                    onChange={(e) => setQuizConfig({ ...quizConfig, passingScore: Number.parseInt(e.target.value) })}
                    className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 text-sm sm:text-base"
                  >
